# Convert an arlequin .arp file to eigenstrat format
# Generate .arp file using fastsimcoal2 -s0 option
# Only DNA sequence supported (with -s0), but
# multiple populations/chromosomes are allowed.
# Ascertained data automatically detected.
# usage: python arlequin2eigenstrat.py -a arp_file.arp(.gz) -o out_root [other options]
# will generate out_root.[snp,ind,geno].
# By default will convert to genotype data, use -p if you want to keep it phased.
...
def parse_options():
    parser = argparse.ArgumentParser(
        description="Convert arlequin .arp file to eigenstrat format."
    )
    parser.add_argument("-a", "--arp", required=True, help="Input .arp file (.gz supported)")
    parser.add_argument("-o", "--out", default="out", help="Output root (default: out)")
    parser.add_argument("-p", "--phased", action="store_true", help="Keep phased (haploid) output instead of converting to genotypes")
...
def load_from_arp(arp):
    ...
    list of lists with the sites on each chromosome, and gt_data which is a
    dict with information on the samples and genotypes.
    """
...
def unphase(gt_data):
    ...
    """
    for sample, data in gt_data.items():
